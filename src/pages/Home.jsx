      ...
      <>
        <img
          ...
          className={` ${
            isHovered
              ? "z-20 opacity-100 "
              : "z-10 opacity-20 laptop:opacity-30"
          } absolute  w-full max-w-full top-0 h-full max-h-full object-cover animate-zoomInOut`}
        />
        <div
          className={` ${
            isHovered
              ? "bg-opacity-80 min-w-[20%] "
              : "bg-opacity-50 min-w-[100%]"
          } transition-all transform duration-300 laptop:absolute mt-32 self-end laptop:mt-0 top-16 left-[200px] z-20 h-[150px] text-peach flex bg-white items-end px-10 p-4 laptop:border-b-2 `}
        >
          <div
            className="w-fit flex-col flex gap-4"
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
          >
            <NavLink to={`/artist/${toSlug(chosenBgImage?.name || "")}`}>
              <h2 className="text-4xl tablet:text-[50px] font-fat laptop:text-[45px] ">
                {chosenBgImage?.name}
              </h2>
            </NavLink>
            <div className="flex gap-1 items-center self-start z-30 group">
              ...
              <NavLink
                to={`/artist/${toSlug(chosenBgImage?.name || "")}`}
        ...
        <div className="z-10 relative h-fit laptop:h-screen flex flex-col gap-8">
          {randomImages?.map((artist, index) => {
            const divPositions = [
              "laptop:z-30 w-full h-[230px] mb-20 tablet:h-[350px] laptop:w-[500px] laptop:h-full top-48 laptop:top-[214px] laptop:left-[20%] object-cover laptop:border-r-2 border-t-2 laptop:border-t-0 laptop:border-l-2 laptop:justify-end animate-slideInLeft laptop:animate-slideUp ",
              "z-20 h-[180px] tablet:h-[200px] laptop:h-[180px] w-screen top-[40%] laptop:top-1/3 right-0 items-end border-t-2 flex-col animate-slideInRight",
              ...
              "z-10 w-full h-[120px] tablet:h-[180px] laptop:w-[180px] laptop:h-screen bottom-[20%] tablet:bottom-1/4 laptop:bottom-0 left-0 laptop:left-2/3 laptop:border-r-2 border-t-2 laptop:border-t-0 items-end laptop:justify-end animate-slideInLeft laptop:animate-slideUp",
            ];
            const imgStyle = [
            ...
            return (
              <div
                className={`${divPositionClass} flex laptop:absolute`}
                key={index}
              >
                <img
